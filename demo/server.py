...
async def predict(file: UploadFile = File(...)):
    ...
    上传一张图片，返回:
      - bpp: 整体 BPP
      - heatmap: base64 编码的 BPP 热力图 PNG
      - channel_bpps: 各通道 BPP
    """
    try:
        import torch
    ...
    if "bpp" in out and out["bpp"] is not None:
        ...
        bpp = ce_loss / math.log(2) * 3

    # 生成简易热力图（仅 iGPT 支持完整 per-position）
    heatmap_b64 = None
    if model_type == "igpt" and out.get("logits") is not None:
        heatmap_b64 = _make_heatmap_b64(model, x, device)
...
def _make_heatmap_b64(model, x, device):
    """生成 32×32 BPP 热力图并返回 base64 PNG。"""
    import torch
    ...
    heatmap = full.reshape(3, 32, 32).sum(axis=0)
