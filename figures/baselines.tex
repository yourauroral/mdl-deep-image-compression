...
%   CC-iGPT v2 (historical experiment; diagnostic protocol) 82.95M/member (K=3) 2.8296
%     R-only ensemble (best+SWA+EMA) + TTA hflip, 6 forwards/image；非正式单模型主表
%   CC-iGPT v2 (formal protocol) 82.95M 2.8328
%     single best.pth + no TTA + teacher-forced；10,000 张
...
\begin{axis}[
    ...
    ylabel near ticks,
    xtick={SparseTrans, PixelSNAIL, ImgTrans, PixelCNNpp, OursHist, OursFormal},
    symbolic x coords={SparseTrans, PixelSNAIL, ImgTrans, PixelCNNpp, OursHist, OursFormal},
    xticklabels={Sparse Trans., PixelSNAIL, Img Trans., PixelCNN++, CC-iGPT(hist.), CC-iGPT(formal)},
...
% 正式 single-checkpoint/no-TTA teacher-forced 柱
\addplot[draw=teal!65!black, fill=teal!24, bar shift=0pt] coordinates {
    (OursFormal, 2.8328)
};
...
\node[font=\scriptsize, align=center, orange!70!black, yshift=4pt] at (axis cs:OursHist, 2.8296) {2.8296\\82.95M/member, K=3};
\node[font=\scriptsize, align=center, teal!65!black, yshift=4pt] at (axis cs:OursFormal, 2.8328) {2.8328\\82.95M};

\end{axis}

% --- 标题 ---
\node[font=\footnotesize\bfseries, anchor=south, yshift=3pt]
    at (current bounding box.north) {CIFAR-10 RGB-bit-exact (formal + historical protocols)};
