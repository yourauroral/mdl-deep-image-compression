...
\node[coarseblk, below=of ctokens] (coarse)
    {\textbf{Coarse iGPT}\;(5M)\\\scriptsize$d{=}256,\ N{=}6,\ h{=}4$\\
     RoPE\,/\,QK-Norm\,/\,SwiGLU};
...
\node[draw, rounded corners=2pt, align=left, font=\scriptsize,
      ...
      right=45mm of bpd, anchor=west] (legend)
{\textbf{Legend}\\[2pt]
 {\color{orange!70!black}\rule[0.5ex]{8mm}{0.7pt}}\,\, bit-exact invariant ctx flow\\
 {\color{green!45!black} $\to$}\, additive embedding injection\\
 $\boldsymbol{+}$\, element-wise addition\\
 $\alpha$\, learnable scalar (shared across positions)\\
 \textit{italic} = key technical features};
