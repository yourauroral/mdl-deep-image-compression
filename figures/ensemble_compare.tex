...
%   前三档 in-training 数字作为 ensemble 收益的视觉参照；全部只作历史诊断。
% 参考线:
%   v1 best+TTA 2.9035        — Phase B 历史诊断
%   PixelSNAIL 2.85           — 文献参考线；与三模型/TTA 配置不是同协议
%   Sparse Transformer 2.80   — 文献参考线；正式比较等待单模型重评
...
\node[font=\footnotesize\bfseries, anchor=south]
    ...
    {CC-iGPT v2 历史诊断溯源 — 三档 in-training 单档 vs Ensemble+TTA};
...
\node[font=\scriptsize, gray!50!black, anchor=north, yshift=-2pt, text width=12cm, align=center]
    ...
    灰色三档为 200ep 训练日志末尾 in-training val（无 TTA）；橙色为三成员、6-forward/image 历史诊断，不是正式单模型主表}};
