...
%       best ep196 = 2.8312 ± 0.0909   (训练曲线最低点)
%       SWA last 31 = 2.8276 ± 0.0909  (Polyak 平均)
%       EMA decay 0.9998 = 2.8301 ± 0.0909
%   Ensemble + TTA: scripts/evaluate.py --ensemble best,swa,ema --tta_hflip 实测 (2026-05-27)
%       2.8296 ± 0.0854
%
% 设计意图: 答辩前时间紧，砍消融端 (best/SWA/EMA × ±TTA = 6 runs)，仅保留主表 1 run。
%   前三档 in-training 数字作为 ensemble 收益的视觉参照 — 灰色显示，强调 ensemble+TTA 是主表数字。
% 参考线:
%   v1 best+TTA 2.9035        — Phase B 主表
%   PixelSNAIL 2.85           — v2 原目标 (已超越)
%   Sparse Transformer 2.80   — v2 上调后目标 (gap 0.030, 逼近未持平)
...
% --- 参考线 3: Sparse Transformer 2.80 (Ours 逼近未持平) ---
...
\node[font=\footnotesize\bfseries, anchor=south]
    ...
    {CC-iGPT v2 主表数字溯源 — 三档 in-training 单档 vs Ensemble+TTA};

% --- 副标题 ---
\node[font=\scriptsize, gray!50!black, anchor=north, yshift=-2pt, text width=12cm, align=center]
    at (current bounding box.south)
    {\textit{N=32/d=448 (82.95M) $\cdot$ EMA 0.9998 $\cdot$ SWA last 31 ckpts (start ep170) $\cdot$ ensemble = log-softmax 平均(best, SWA, EMA) $\cdot$ TTA = hflip 双 forward 取均\\
    灰色三档为 200ep 训练日志末尾 in-training val（无 TTA），橙色为最终主表数字（ensemble + TTA hflip）}};
