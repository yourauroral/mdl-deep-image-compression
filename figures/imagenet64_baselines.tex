...
%   CC-iGPT (Ours)      3.4800 本工作，IN64 v1 12ep，主表 ensemble(best+SWA+EMA)+TTA hflip（4卡实测 2026-06-07）
%
% 主表数字 3.4800 ± 0.1161（ensemble best/SWA/EMA + TTA hflip；单 ckpt best+TTA=3.4810）。
%   超 SPN 3.52 by 0.040、逼近但未达 Sparse Transformer 152M 3.44（差 0.040）。叙事"超 SPN、逼近 Sparse Transformer"。
%   注：± 0.1161 是 4卡 batch-level std，与 CIFAR 主表 per-image 口径不同（要可比需单卡 --per_image_stats 重取）。
...
% 第5个橘黄柱子（Ours，主表 ensemble+TTA 3.4800）
...
\node[font=\scriptsize\itshape, gray!50!black, anchor=north, yshift=-2pt]
    ...
    {CC-iGPT v1 12ep: ensemble(best+SWA+EMA)+TTA = 3.4800 bpd (4-GPU)};
