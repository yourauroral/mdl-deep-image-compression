...
%   CC-iGPT (Ours)      3.4931 本工作，IN64 v1 in-training ep9（占位，ep10-12 退火尾段未走完，最终待回填）
%
% ！占位说明：3.4931 是 ep9/12 末 val（cosine 退火 ep10-12 尚未走完），非最终数字。已低于 SPN 3.52、距目标 3.44 剩 0.0531。
%   训练完成后用最终 val/ensemble 数字替换 Ours 柱高与标注。
...
% 第5个橘黄柱子（Ours，占位 ep9 val 3.4931）
...
\node[font=\scriptsize, align=center, orange!70!black, yshift=4pt] at (axis cs:Ours, 3.4931) {\textbf{3.49}\,\scriptsize ep9*};
...
\node[font=\scriptsize\itshape, gray!50!black, anchor=north, yshift=-2pt]
    ...
    {* CC-iGPT v1 in-training ep9/12 (cosine anneal tail ep10--12 not yet run); final number TBD};
