...
%
% 数据来源: experiments/ccigpt_imagenet64_v1_train.log（截至 ep8/12 完成 + ep9 训练中，2026-06-04）
...
%
% ！进行中：ep9-12 待回填（ep9 训练中 step18300/26690 ≈ 69%）。val 已低于 SPN 3.52，距目标 3.44 剩 0.0594；
%   cosine 退火尾段已在走（ep5→8 LR 2.51→1.37e-4），最终 3.44 判定待 ep12（ETA 06-07~08）。
...
\node[font=\scriptsize, purple!45!black, anchor=north, align=center]
    at (axis cs:10.75, 4.45) {cosine anneal tail\\(ep9--12, not yet run)};

% --- 当前进度竖虚线 (ep8 done / ep9 training) ---
\draw[densely dashed, gray!45!orange, line width=0.4pt]
    (axis cs:8, 3.35) -- (axis cs:8, 4.5);
\node[font=\scriptsize, orange!55!black, anchor=south west, rotate=90]
    at (axis cs:8, 3.55) {\,now: ep8 done};
...
\node[font=\scriptsize, orange!80!black, anchor=south west]
    at (axis cs:8.05, 3.4994) {ep8 val \textbf{3.4994}};
...
\node[font=\scriptsize, gray!50!black, anchor=north, yshift=-2pt]
    ...
    {\textit{N=32/d=448 $\cdot$ 12ep cosine $\cdot$ warmup 1ep $\cdot$ effective batch 288 $\cdot$ 4-GPU DDP $\cdot$ as of ep8/12 done, ep9 training (2026-06-04)}};
