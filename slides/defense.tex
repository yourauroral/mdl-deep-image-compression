...
\begin{tabular}{@{}llll@{}}
\toprule
\textbf{侧面} & \textbf{任务} & \textbf{验证的 MDL 命题} & \textbf{状态} \\
\midrule
表征 & Linear probe       & 线性可分表征更强        & \alert{\textbf{已实测}} \\
密度 & OOD typicality     & 能识别分布外样本        & \alert{\textbf{已实测}} \\
泛化 & 跨数据集 bpd       & 学自然图像统计、非记忆  & \alert{\textbf{已实测}} \\
生成 & 图像补全 (inpaint) & AR 条件生成能力         & \alert{\textbf{已实测}} \\
\midrule
\multicolumn{4}{@{}l}{\textbf{地基}\;：真实可解性 roundtrip —— 算术编解码逐像素 bit-identical，} \\
\multicolumn{4}{@{}l}{\quad\quad\;\; 证明所报 bpd 是\alert{真实可逆码长}而非 proxy} \\
\bottomrule
\end{tabular}
...
本工作用 MDL 把两者串成一条因果链。「代码就绪」者复用 CIFAR v2 现成 ckpt，待 GPU 空窗实测。
\end{frame}

% ============================== 下游 ① 表征：Linear probe (图) ==============================
\begin{frame}{下游 ① 表征：Linear probe（逐层 top-1 准确率）}
\figplaceholder{linear-probe-layerwise-acc.pdf}{11}{6}
\end{frame}

% ============================== 下游 ① 表征：Linear probe (文字) ==============================
\begin{frame}{下游 ① 表征：probe 方法与结果}
\begin{columns}[T]
\begin{column}{0.50\textwidth}
\small
\textbf{方法}
...
% ============================== 下游 ② 密度：OOD typicality (图) ==============================
\begin{frame}{下游 ② 密度：OOD typicality（AUROC 对比）}
\figplaceholder{ood-typicality-auroc.pdf}{11}{6}
\end{frame}

% ============================== 下游 ② 密度：OOD typicality (文字) ==============================
\begin{frame}{下游 ② 密度：typicality 方法与差异化}
\begin{columns}[T]
\begin{column}{0.50\textwidth}
\small
\textbf{方法}
...
% ============================== 下游 ③ 泛化：跨数据集 bpd (图) ==============================
\begin{frame}{下游 ③ 泛化：跨数据集 bpd}
\figplaceholder{cross-dataset-bpd.pdf}{11}{6}
\end{frame}

% ============================== 下游 ③ 泛化：跨数据集 bpd (文字) ==============================
\begin{frame}{下游 ③ 泛化：跨数据集 bpd 方法与命题}
\begin{columns}[T]
\begin{column}{0.50\textwidth}
\small
\textbf{方法}
...
% ============================== 下游 ④ 生成：图像补全 (图) ==============================
\begin{frame}{下游 ④ 生成：图像补全（AR inpainting）}
...
% ============================== 下游 ⑤ 地基：真实可解性 roundtrip (图) ==============================
\begin{frame}{下游 ⑤ 地基：真实可解性 roundtrip}
\figplaceholder{lossless-roundtrip.pdf}{11}{6}
\end{frame}

% ============================== 下游 ⑤ 地基：真实可解性 roundtrip (文字) ==============================
\begin{frame}{下游 ⑤ 地基：可解性方法与命题}
\begin{columns}[T]
\begin{column}{0.50\textwidth}
\small
\textbf{方法}
