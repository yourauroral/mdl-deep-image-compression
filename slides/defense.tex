...
密度 & OOD typicality     & 能识别分布外样本        & 代码就绪 \\
泛化 & 跨数据集 bpd       & 学自然图像统计、非记忆  & 代码就绪 \\
生成 & 图像补全 (inpaint) & AR 条件生成能力         & 代码就绪 \\
\midrule
...
\]
\begin{itemize}
  \item coarse / fine 两个独立 typicality 信号
  \item 单尺度模型给不出
\end{itemize}
...
\begin{itemize}\setlength{\itemsep}{2pt}
  ...
  \item SVHN: raw \fillme{x.xx} / typ \fillme{x.xx} / dual \fillme{x.xx}
  \item CIFAR-100: raw \fillme{x.xx} / typ \fillme{x.xx} / dual \fillme{x.xx}
\end{itemize}
\end{column}
\end{columns}
\end{frame}
...
\begin{itemize}\setlength{\itemsep}{2pt}
  \item CIFAR-10 (in-domain): \fillme{x.xx}
  \item CIFAR-100: \fillme{x.xx} \;/\; SVHN: \fillme{x.xx} \;/\; STL-10: \fillme{x.xx}
\end{itemize}
\end{column}
\end{columns}
\end{frame}
...
\figplaceholder{image-completion-grid.pdf}{11}{6}
\end{frame}
...
\begin{itemize}\setlength{\itemsep}{2pt}
  ...
  \item 补全网格见左图 \fillme{回填 image-completion-grid.pdf}
\end{itemize}
\end{column}
\end{columns}
\end{frame}
