...
\begin{column}{0.50\textwidth}
\centering
\includegraphics[width=\linewidth,keepaspectratio]{ImageNet64baseline.pdf}
\end{column}
\begin{column}{0.50\textwidth}
\centering
\includegraphics[width=\linewidth,keepaspectratio]{ImageNet_trainning_curve.pdf}
\end{column}
\end{columns}

\vspace{1mm}
\centering\scriptsize\color{muted}
同款 CC-iGPT 双尺度（coarse $16{\times}16{=}256$ token $+$ fine 12288 token），4-GPU DDP、12 ep cosine。\\
in-training ep5/12 val \textbf{3.5217}，已追平 SPN (3.52)；cosine 退火尾段（ep8--12）未走，目标 \alert{$<$3.44}（Sparse Transformer）待最终判定。
\end{frame}
...
\midrule
\multicolumn{3}{@{}l}{\textbf{地基}\;：真实可解性 —— 图像 ⇄ .bin 逐 token gold 编解码 bit-identical，} \\
...
% ============================== 下游 3 地基：真实可解性 (图) ==============================
\begin{frame}{下游 3 地基：真实可解性 —— 图像 ⇄ .bin 编解码}
\begin{center}
...
\begin{itemize}\setlength{\itemsep}{2pt}
  ...
  \item 逐像素断言 bit-identical（前端可现场上传图 ⇄ 下载 .bin）
\end{itemize}
...
\begin{itemize}\setlength{\itemsep}{2pt}
  ...
  \item \textbf{地基} —— 真实可解性（图像 ⇄ .bin bit-identical）
\end{itemize}
